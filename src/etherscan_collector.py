"""
Etherscan Data Collector Module

This module uses the Etherscan V2 API to fetch accurate blockchain data for revenue analysis.
Etherscan provides data for multiple EVM chains including Ethereum, Arbitrum, Optimism, etc.
"""
...
ETHERSCAN_BASE_URLS = {
    "ethereum": "https://api.etherscan.io/v2",
    "arbitrum": "https://api.arbiscan.io/v2",
    "optimism": "https://api.optimistic.etherscan.io/v2",
    "polygon": "https://api.polygonscan.com/v2",
    "base": "https://api.basescan.org/v2",
    "avalanche": "https://api.snowtrace.io/v2",
    "bsc": "https://api.bscscan.com/v2",
    "fantom": "https://api.ftmscan.com/v2",
}
...
class EtherscanCollector:
    def __init__(self, api_key=ETHERSCAN_API_KEY, data_dir="../data"):
        """Initialize the EtherscanCollector."""
    ...
    def get_supported_chains(self):
        """Get list of supported chains from Etherscan V2 API."""
        print("Fetching supported chains from Etherscan V2 API...")
        
        url = f"{ETHERSCAN_BASE_URLS['ethereum']}/chainlist"
        params = {
            "apikey": self.api_key
        }
    ...
    def get_contract_events(self, contract_address, chain="ethereum", start_block=0, end_block=99999999):
        """
        Get events for a contract address.
        ...
        """
        print(f"Fetching {chain} contract events for {contract_address}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "1":
                    events = pd.DataFrame(data.get("result", []))
            ...
            print(f"Error fetching contract events: {e}")
    ...
    def get_contract_internal_transactions(self, contract_address, chain="ethereum", start_block=0, end_block=99999999):
        ...
        # Build URL
        url = f"{base_url}/account/txlistinternal"
    ...
    def collect_protocol_data(self, protocol_name, chain="ethereum", start_date=None, end_date=None):
        """
    ...
    def collect_all_protocols_data(self, start_date=None, end_date=None):
        ...
        for protocol in PROTOCOLS:
            ...
            # Skip non-Ethereum protocols for now
            if protocol_name not in PROTOCOL_CONTRACTS:
                print(f"Skipping {protocol_name} - no contract addresses defined")
                continue
            
            # Process each chain for the protocol
            for chain in PROTOCOL_CONTRACTS.get(protocol_name, {}):
                protocol_data = self.collect_protocol_data(
                    protocol_name, chain, start_date, end_date
                )
...
if __name__ == "__main__":
    ...
    # Collect data for Aave on Ethereum
    aave_data = collector.collect_protocol_data("Aave", "ethereum")
